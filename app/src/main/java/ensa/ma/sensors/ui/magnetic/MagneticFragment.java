...
public class MagneticFragment extends Fragment implements SensorEventListener {
    ...
    private boolean realVariationDetected = false;
    ...
    public View onCreateView(
        ...
        valueText.setText("Initialisation du champ magnétique...");
    ...
    private void configureChart() {
        ...
        description.setText("Champ magnétique");
    ...
    public void onResume() {
        ...
        if (magneticSensor != null) {
            ...
            valueText.setText("Capteur détecté. Attente des variations...");

            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    if (isAdded() && (!realValueReceived || !realVariationDetected)) {
                        valueText.setText("Valeur stable. Simulation magnétique activée.");
                        startSimulation();
                    }
                }
            }, 3000);
            ...
            valueText.setText("Capteur magnétique absent. Simulation activée.");
            startSimulation();
        }
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (lastMagnitude > 0 && Math.abs(magnitude - lastMagnitude) > 0.5f) {
            realVariationDetected = true;
        }

        lastMagnitude = magnitude;

        valueText.setText(
                "X : " + String.format("%.2f", x) + " µT\n"
                        + "Y : " + String.format("%.2f", y) + " µT\n"
                        + "Z : " + String.format("%.2f", z) + " µT\n"
                        + "Norme : " + String.format("%.2f", magnitude) + " µT"
        );

        addPointToChart(magnitude);
    }
    ...
    private void startSimulation() {
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                float fakeMagnetic =
                        45f + (float) Math.sin(simulationTime / 3f) * 10f;

                valueText.setText(
                        "Champ magnétique simulé : "
                                + String.format("%.2f", fakeMagnetic)
                                + " µT"
                );

                addPointToChart(fakeMagnetic);
    ...
    private void addPointToChart(float value) {
        ...
        if (set == null) {
            set = new LineDataSet(new ArrayList<Entry>(), "Norme magnétique µT");
